...
const QUEST_TYPES = [
    ...
    { type: 'EXAM_COMPLETE', title: 'এক্সাম ওয়ারিয়র', desc: '৩টি মডেল টেস্ট সম্পন্ন করো', target: 3, reward: 100, icon: 'FileCheck' },
...
const questionBankSchema = new mongoose.Schema({
  ...
  topic: String,
  question: { type: String, required: true }, // Not indexed to allow long text, lookup done by ID mostly
...
// OPTIMIZED: Uses Reference instead of embedding
...
// Helper to find existing question or create new one (Deduplication)
const getOrInsertQuestion = async (questionData) => {
    // If it has an _id and exists in DB, return that ID
    if (questionData._id && mongoose.isValidObjectId(questionData._id)) {
        return questionData._id;
    }

    // Attempt to find by exact question text and options to avoid duplicates
    // Using simple text matching for now. Ideally should hash.
...
app.post('/api/users/:userId/exam-results', async (req, res) => {
    try {
        ...
        if (isDbConnected()) {
            ...
            // Save Mistakes using optimized References
            if (resultData.mistakes && resultData.mistakes.length > 0) {
                for (const m of resultData.mistakes) {
                    // 1. Get or Create Question ID
                    const qId = await getOrInsertQuestion(m);
                    
                    // 2. Update Mistake Record
...
app.post('/api/users/:userId/saved-questions', async (req, res) => {
    try {
        const { questionId, question, folder } = req.body;
        
        if (isDbConnected()) {
            // Logic: If 'question' object is provided (AI question), create it first
            let finalQId = questionId;
            
            if (!questionId && question) {
                finalQId = await getOrInsertQuestion(question);
            }

            if (finalQId) {
                // Check if already saved
                const existing = await SavedQuestion.findOne({ userId: req.params.userId, questionId: finalQId });
                if (!existing) {
                    await SavedQuestion.create({ userId: req.params.userId, questionId: finalQId, folder });
                }
            }
        } else {
    ...
    try {
        if (isDbConnected()) {
            const mistakes = await Mistake.find({ userId: req.params.userId })
                                          ...
                                          .limit(100)
                                          .lean();
            res.json(mistakes.filter(m => m.questionId)); // Remove null refs
        } else {
